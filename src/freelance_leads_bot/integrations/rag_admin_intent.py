...
def _intent_prompt(command: str, context: dict[str, Any]) -> str:
    return (
        ...
        "service_disable, service_delete, avito_reminder_cadence_update, unknown.\n"
        "scope may include: service, product, city, volume_ml, topic.\n"
        "operation may include: type, value, old_value, new_value, title, status, interval_seconds, escalation_seconds.\n"
        "Use avito_reminder_cadence_update only when Olga/admin wants to change how often Avito reminders repeat; "
        "convert intervals like 'twice a day', 'every six hours', or Russian equivalents to interval_seconds.\n"
