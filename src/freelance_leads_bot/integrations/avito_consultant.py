...
def _codex_payload_reply_rules(profile: RoleProfile) -> list[str]:
    if profile.role in {CodexRole.AVITO_CLIENT, CodexRole.TELEGRAM_CLIENT, CodexRole.VK_CLIENT}:
        return [
            ...
            "Не склоняй клиента на консультацию как стандартный шаг; если нужна оценка Ольги, не додумывай за неё и попроси телефон для связи или недостающие фото/данные без обещаний результата.",
