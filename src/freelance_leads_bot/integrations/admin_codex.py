        ...
        return {
            ...
            "reply_rules": [
                *profile.reply_rules,
                ...
                "Если Ольга/админ свободной фразой просит изменить память/правила/цены/сроки RAG ('подними цены на 10%', 'это устарело', 'не говори про очную консультацию', 'запомни вот так'), сначала вызови expert_rag.plan_change. Не применяй изменения без подтверждения отображённого плана.",
                "Если Ольга/админ явно подтверждает уже показанный RAG-план, используй expert_rag.apply_plan; если правит карточку плана, пересобери plan_change с её правкой.",
...
def build_admin_codex_prompt(payload: dict[str, Any], trace: list[dict[str, Any]]) -> str:
    ...
    return (
        ...
        "Если Ольга просит изменить RAG-память, цены, сроки эффекта или правила ответов в свободной форме, вызови expert_rag.plan_change и затем верни короткий ответ, что подготовлен план на подтверждение. Не вызывай expert_rag.apply_plan, пока она явно не подтвердила уже показанный план.\n"
        "Выбор tools в основном admin-Codex пути делает Codex; для expert_rag.plan_change приложение дополнительно валидирует структурированный intent перед созданием pending plan.\n"
