...
def _codex_payload_reply_rules(profile: RoleProfile) -> list[str]:
    if profile.role in {CodexRole.AVITO_CLIENT, CodexRole.TELEGRAM_CLIENT, CodexRole.VK_CLIENT}:
        return [
            ...
            "Не предлагай очную консультацию как стандартный шаг; если нужна оценка, предложи онлайн-разбор и собери зону, желаемый результат, объём, прошлые процедуры и фото.",
