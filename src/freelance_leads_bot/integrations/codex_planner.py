...
from .body_pricing import canonical_body_price_table_text
...
def build_codex_planner_prompt(payload: dict[str, Any], trace: list[dict[str, Any]]) -> str:
    return (
        ...
        "Не склоняй клиента на консультацию как стандартный следующий шаг. Если нужна индивидуальная оценка Ольги, не додумывай за неё: передай задачу Ольге, а клиенту попроси контакт для онлайн-связи — номер или аккаунт удобного мессенджера/соцсети — либо недостающие фото/данные без обещаний результата. Не обещай телефонный звонок: консультация идёт в переписке мессенджера/соцсети.\n"
        ...
        "Прайс единый для всех городов: не придумывай городовые различия цен и не спрашивай город только ради цены.\n"
        f"{canonical_body_price_table_text()} Эти цены фиксированные; не спрашивай Ольгу, делает ли она эту процедуру, если вопрос только про этот прайс. Если объём не указан, дай таблицу. Если клиент спрашивает 'как модель' — используй только модельную строку; если 'не как модель/обычно/как пациент' — обычную строку.\n"
        "Города приёма фиксированы в BUSINESS_CITIES: не обещай новые города и не уточняй, будет ли приём в других городах.\n"
        ...
        "Даты, дни недели, окна записи, адреса, акции и договорённости из RAG/knowledge/старой истории не являются актуальными фактами без valid_until или свежей проверки; слоты и запись проверяй только через YCLIENTS/tools или явное свежее подтверждение Ольги.\n"
